...
<GradientAnimation>
	<div class="mt-[4.3rem] flex flex-col gap-5 text-white md:flex-row">
					...
					{#each eligData2 as item}
						<div class="team-regs mr-5 flex flex-row pb-2">
							...
							<div class="team-reg-det flex flex-col">
								<div class="team-reg-text px-1 font-LeagueSpartanFont text-[0.8rem]">
									{item.title}
								</div>
								<div class="team-reg-data mx-1 flex justify-start text-[0.95rem]">{item.des}</div>
							</div>
						</div>
					{/each}
				</div>
		...
		<div class="neurathon-info-container g-1 overflow-hidden md:w-[60%] lg:w-[60%] xl:w-[60%] mb-3">
