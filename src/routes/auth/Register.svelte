<script lang="ts">
	...
	const validate = () => {
		...
		if (!form.name) formErrors.nameError = 'Name is required';
		else if (!/^[A-Za-z\s]+$/.test(form.name))
			formErrors.nameError = 'Name can only contain letters and spaces';
		else if (form.name.split(' ').length < 2) formErrors.nameError = 'Full name required';

		if (!form.email) formErrors.emailError = 'Email is required';
		else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))
			formErrors.emailError = 'Enter a valid email address';

		if (!form.phone) formErrors.phoneError = 'Phone number is required';
	...
	<div class="w-full overflow-hidden">
		...
		>
			<div class="text-md flex items-center gap-4">
				<label class="flex font-bold text-slate-300" for="username"> Name </label>
				<input
			...
			<div class="text-md mt-4 flex items-center gap-4">
				<label class="flex font-bold text-slate-300" for="email"> Email </label>
				<input
					class="focus:shadow-outline w-full appearance-none rounded border border-slate-400 bg-slate-700 p-2 leading-tight text-slate-400 opacity-80 shadow focus:outline-none"
			...
			{/if}
			<div class="text-md mt-4 flex items-center gap-4">
				<label class="flex font-bold text-slate-300" for="phone"> Phone No. </label>
				<input
					class="focus:shadow-outline w-full appearance-none rounded border border-slate-400 bg-slate-700 p-2 leading-tight text-slate-400 opacity-80 shadow focus:outline-none"
			...
			{/if}
			<div class="text-md mt-4 flex items-center gap-4">
				<label class="flex font-bold text-slate-300" for="password"> Password </label>
				<input
					class="focus:shadow-outline w-full appearance-none rounded border border-slate-400 bg-slate-700 p-2 leading-tight text-slate-400 opacity-80 shadow focus:outline-none"
